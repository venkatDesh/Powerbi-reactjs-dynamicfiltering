...
import * as emb from "../components/ViewReports/EmbeddedReport"
...
export default class PowerBiEmbeddingService {
  ...
  static embedReport(report: PowerBiReport, embedContainer: HTMLElement): powerbi.Report {
    ...
    var accessToken: string = SpaAuthService.accessToken;
    var models = pbimodels;
    var Urlpart=embedUrl.split("&");
    //var y=(document.getElementById("selectSlicer_Cat") as HTMLInputElement).value;
    var config: any = {
      type: 'report',
      id: embedReportId,
      embedUrl:  Urlpart[0]+"&pageName=ReportSection"+"&$filter=Product/Category eq '"+emb.selVal_cat+"' and Date/Year eq '"+emb.selVal+"'",
      accessToken: accessToken,
      tokenType: models.TokenType.Aad,
